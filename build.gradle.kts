...
intellijPlatform {
    // This plugin contributes only themes and editor schemes — no Configurable, so
    // there is nothing for the Settings search index to find. Leaving it on also
    // breaks containerised builds: the task starts a headless IDE that binds a Unix
    // domain socket for its sandbox directory lock, which fails with
    // "SocketException: Operation not supported" when build/ sits on a bind mount
    // (Docker/Podman on Windows or macOS).
